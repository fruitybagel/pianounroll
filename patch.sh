...
printf '\x1F\x20\x03\xD5' | dd of="${libpath}.new" bs=1 seek=$((0x${arm64_symbol_offset} + $arm64_offset + 4 * 4)) conv=notrunc 2>/dev/null
...
# dd if=$libpath.new bs=1 skip=$((0x${x86_64_symbol_offset} + $x86_64_offset))  count=32 2>/dev/null | hexdump -C

# dd if=$libpath.new bs=1 skip=$((0x${arm64_symbol_offset} + $arm64_offset))  count=32 2>/dev/null | hexdump -C
